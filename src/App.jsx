...
function ConciergePanel({ kind, nightlifeDestination }) {
  ...
  const title = nightlifePanel ? "Go Out With Visit Malindi" : "Book / Enquire With Visit Malindi";
  ...
  return <section className={`concierge-panel ${nightlifePanel ? "concierge-panel-night" : ""}`}><div><span className="eyebrow">{nightlifePanel ? "After-dark concierge" : "Dining concierge"}</span><h2>{title}</h2><p>{text}</p><WhatsAppButton type={nightlifePanel ? "nightlife" : "food"} name={nightlifePanel ? nightlifeDestination : "a restaurant or venue"} label={nightlifePanel ? "Go Out With Visit Malindi" : "Book / Enquire with Visit Malindi"} /></div><ul>{services.map((service) => <li key={service}><span>✓</span>{service}</li>)}</ul></section>;
}
