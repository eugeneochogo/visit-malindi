...
function ListingPage({ config }) {
  ...
  useEffect(() => { setFilter("All"); setLocation("All"); setExperienceType("All"); }, [config.title]);
  ...
  const visible = scopedData.filter((item) => {
    ...
    const categoryMatch = filter === "All" || [item.category, item.duration, ...(item.tags || [])].filter(Boolean).some((value) => value.toLowerCase() === normalizedFilter || value.toLowerCase().includes(normalizedFilter));
    return categoryMatch && (location === "All" || item.location.includes(location)) && (experienceType === "All" || item.experienceTypes?.includes(experienceType));
  });
